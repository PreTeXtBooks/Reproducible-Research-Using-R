...
Now, that works, but in Quarto, there is an even better way to display tables. Using the `knitr` package (@knitr), we can display tables with the `kable()` function, which is purposely dedicated to displaying tabular data in Quarto.
