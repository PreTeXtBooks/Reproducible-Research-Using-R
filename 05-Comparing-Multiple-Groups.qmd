...
Instead of writing all of the code manually, we can utilize the `favstats()` command from the `mosaic` package (@mosaic).
...
We can also utilize the `supernova()` function, from the `supernova` package (@supernova), for a cleaner output.
...
Using the `aictab()` function from the `AICcmodavg` package (@AICcmodavg), we can compare the one-way ANOVA model with the two-way ANOVA model to see what is the overall better model of understanding differences in memory scores.
