...
Within `ggplot2`, functions like `labs()` allow you to clearly label axes, titles, and legends so the intent of the plot is immediately clear. When working in R Markdown or bookdown (@R-sec-r-markdown-graphs), figure captions (using `fig-cap`) provide additional context that travels with the figure wherever it appears.
