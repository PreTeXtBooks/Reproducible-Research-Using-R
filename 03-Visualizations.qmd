...
You may be thinking "how powerful can a visualization even be?" That is a great question, that Anscombe’s quartet will help answer (@quartets).
...
One of the strongest qualities in R is its ability to create visualizations, powered by `ggplot2` (@ggplot2). However, it is possible to use base R to create plots as well. It is recommended to use ggplot2; however, you may encounter base R plots in older scripts or documentation, so an example is included for familiarity.
