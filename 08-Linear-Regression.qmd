...
If we wanted to use another package instead of ggplot, we could also use the library `ggformula` (@ggformula), whihc has `gf_point()`.
...
In R, we always want to explore different packages. The `broom` package (@broom) in R is very helpful when making statistical objects into nicer tibbles, usig the `tidy()` function.
...
The Breusch-Pagan test provides insights for this. To perform this in R, we will use the `bptest()` function from the `lmtest` package (@lmtest).
