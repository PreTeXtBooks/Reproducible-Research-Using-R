...
In order to install packages, we can utilize the `install.packages()` command. Let's install our first official package, the `tidyverse` package (@tidyverse).
...
The `palmerpenguins` package (@palmerpenguins) we will be using in this chapter. Before then, I want to show you that R is fun and diverse. The package `fortunes` (@fortunes) provides you a random R quote with the function `fortune()` and the `say()` command from the `cowsay` package (@cowsay) provides a beautiful animal saying whatever you want it to say. Below the animal is "cow" but try some other ones (I suggest "dragon").
...
## Key Functions & Commands {#sec-tidyverse-keyfunctions}
