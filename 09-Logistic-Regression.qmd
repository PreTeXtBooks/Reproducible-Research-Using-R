...
First, we need to load the `ISLR2` (@ISLR2) package and then we will create our `cc_data` dataset using the `Default` dataset, which inside the package.
...
Our goal: use a decent portion of our real data to test our model, and compare it to data not used in the model. To help with this, we will be using the `sample.split()` function from the `caTools` pakage (@caTools).
...
Unlike in linear regression, logistic regression does not have an R²: (for a review on R²: see @sec-linear-reg-model). However, we can use the `pR2()` function from the `pscl` library (@pscl) to calculate a pseudo R² value.
...
With the `varImp()` function inside the `caret` package (@caret), we can also see which variables are the most important in our regression model.
...
Just like in @sec-partial-cor, we want to check to see if there is any interactions between some of our variables that may be impacting our regression model. Using the `vif()` function from the `car` library (@car), we can see the interactions between our variables in our model.
