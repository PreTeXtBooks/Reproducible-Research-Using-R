...
We can get even fancier and use the command `ggpairs` from the `GGally` package (@GGally).
...
To account for this, we can utilize the `ppcor` library (@ppcor) and run a partial correlation using the `pcor.test()` command.
