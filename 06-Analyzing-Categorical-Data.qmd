...
Similarly to how we loaded data in @sec-loading-data, we are going to load the `Infection Treatments.xlsx` dataset into R using the `readxl` package (@readxl) and the `read_xlsx()` function inside of it.

Instead of using the `summary()` or `str()` functions, we are going to experiment with the `skim()` function from the `skimr` package (@skimr) to investigate our data.
...
We will call on the help of the `janitor` package (@janitor), specifically the `tabyl()`, `adorn_totals()`, `adorn_percentages()`, `adorn_pct_formatting()`, and `adorn_ns()` functions.
...
If you’d like to enhance the aesthetic of your plots, consider using the `ggthemes` package (@ggthemes). It provides a variety of alternative themes that can quickly change the look and feel of your graphics.
...
Using the `patchwork` package (@patchwork), we can add two or more plots together using the `+` symbol.
...
To make our graphs more interactive with our users, we can utilize the `ggplotly()` function from the `plotly` package (@plotly) (@webshot2).
...
In the `gmodels` package (@gmodels) we are able to utilize the `CrossTable()` command. *Beware*, this can at first be overwhelming, as a lot of information is thrown at you.
...
You already know that visualizations really help paint the picture, and *chi-square contributions* are not exempt. For this, the `pheatmap` package (@pheatmap) comes in handy with the `pheatmap()` function..
...
Cramer’s V tells you the strength of the relationship between two categorical variables, similar to correlation coefficient It is important to note that this measures effect size. This is **not** a statistic. We can utilize the `cramerV()` command from the `rcompanion` package (@cramerV).
