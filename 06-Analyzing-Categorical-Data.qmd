...
Similarly to how we loaded data in @sec-loading-data, we are going to load the `Infection Treatments.xlsx` dataset into R using the `readxl` package (@R-readxl) and the `read_xlsx()` function inside of it.
...
Cramer’s V tells you the strength of the relationship between two categorical variables, similar to correlation coefficient It is important to note that this measures effect size. This is **not** a statistic. We can utilize the `cramerV()` command from the `rcompanion` package (@R-cramerV).
